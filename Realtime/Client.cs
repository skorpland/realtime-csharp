...
public class Client : IRealtimeClient<RealtimeSocket, RealtimeChannel>
{
    ...
    /// <summary>
    /// Sets the current Access Token every heartbeat (see: https://github.com/skorpland/realtime-js/blob/59bd47956ebe4e23b3e1a6c07f5fe2cfe943e8ad/src/RealtimeClient.ts#L437)
    /// </summary>
    ...
    /// Ref: https://github.com/skorpland/realtime-js/pull/117 | https://github.com/skorpland/realtime-js/pull/117
    /// </summary>
    ...
    public void SetAuth(string jwt)
    {
        ...
        {
            // See: https://github.com/skorpland/realtime-js/pull/126
    ...
    /// Adds a RealtimeChannel subscription - if a subscription exists with the same signature, the existing subscription will be returned.
    /// </summary>
    /// <param name="database">Database to connect to, with Powerbase this will likely be `realtime`.</param>
    ...
    private void HandleSocketStateChanged(IRealtimeSocket sender, SocketState state)
    {
        switch (state)
        {
            case SocketState.Open:
                // Ref: https://github.com/skorpland/realtime-js/pull/116/files
                if (!string.IsNullOrEmpty(AccessToken))
                    SetAuth(AccessToken!);
